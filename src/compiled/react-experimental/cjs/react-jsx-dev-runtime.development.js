...
if (process.env.NODE_ENV !== 'production') {
  (function () {
    ...
    // -----------------------------------------------------------------------------
    let enableTransitionTracing = false; // No known bugs, but needs performance testing
    // stuff. Intended to enable React core members to more easily debug scheduling
    // issues in DEV builds.

    let enableDebugTracing = false; // Track which Fiber(s) schedule render work.

    let REACT_CLIENT_REFERENCE$1 = Symbol.for('react.client.reference');
    ...
    function warnIfStringRefCannotBeAutoConverted(config, self) {
      {
        if (
          ...
          let componentName = getComponentNameFromType(
    ...
    function jsxWithValidation(
    ...
    ) {
      {
        ...
        let element = jsxDEV$1(type, props, key, source, self); // The result can be nullish if a mock or a custom function is used.
        ...
        if (hasOwnProperty.call(props, 'key')) {
          let componentName = getComponentNameFromType(type);
          let keys = Object.keys(props).filter(function (k) {
            return k !== 'key';
          });
          let beforeExample =
            keys.length > 0
