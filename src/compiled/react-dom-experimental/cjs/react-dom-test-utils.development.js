...
if (process.env.NODE_ENV !== 'production') {
  (function () {
    ...
    let HostComponent = 5;
    let HostText = 6;
    let HostHoistable = 26;
    let HostSingleton = 27;
    ...
    let UIEventInterface = assign({}, EventInterface, {
      view: 0,
      detail: 0,
    });
    ...
    let MouseEventInterface = assign({}, UIEventInterface, {
      screenX: 0,
      screenY: 0,
      clientX: 0,
      clientY: 0,
    ...
    let DragEventInterface = assign({}, MouseEventInterface, {
      dataTransfer: 0,
    });
    ...
    let FocusEventInterface = assign({}, UIEventInterface, {
      relatedTarget: 0,
    });
    ...
    let AnimationEventInterface = assign({}, EventInterface, {
      animationName: 0,
      elapsedTime: 0,
      pseudoElement: 0,
    });
    ...
    let ClipboardEventInterface = assign({}, EventInterface, {
    ...
    let CompositionEventInterface = assign({}, EventInterface, {
      data: 0,
    });
    ...
    function getEventKey(nativeEvent) {
      if (nativeEvent.key) {
        ...
        let key = normalizeKey[nativeEvent.key] || nativeEvent.key; // $FlowFixMe[invalid-computed-prop] unable to index with a `mixed` value
    ...
    let PointerEventInterface = assign({}, MouseEventInterface, {
      pointerId: 0,
      width: 0,
      height: 0,
      pressure: 0,
    ...
    let TouchEventInterface = assign({}, UIEventInterface, {
      touches: 0,
      targetTouches: 0,
      changedTouches: 0,
      altKey: 0,
    ...
    let TransitionEventInterface = assign({}, EventInterface, {
      propertyName: 0,
      elapsedTime: 0,
      pseudoElement: 0,
    });
    ...
    let WheelEventInterface = assign({}, MouseEventInterface, {
    ...
    function invokeGuardedCallbackImpl(name, func, context) {
      {
        ...
        if (fakeNode) {
          ...
          let windowEventDescriptor = Object.getOwnPropertyDescriptor(
            window,
            'event'
          );

          let restoreAfterDispatch = function () {
          ...
          let _funcArgs = Array.prototype.slice.call(arguments, 3);
    ...
    let EventInternals = SecretInternals.Events;
    let getInstanceFromNode = EventInternals[0];
    let getNodeFromInstance = EventInternals[1];
    let getFiberCurrentPropsFromNode = EventInternals[2];
    let enqueueStateRestore = EventInternals[3];
    let restoreStateIfNeeded = EventInternals[4];
    let act = React.unstable_act;
    ...
    function findAllInRenderedFiberTreeInternal(fiber, test) {
      ...
      let currentParent = findCurrentFiberUsingSlowPath(fiber);
    ...
    function renderIntoDocument(element) {
      let div = document.createElement('div'); // None of our tests actually require attaching the container to the
    ...
    function isCompositeComponentWithType(inst, type) {
      ...
      let internalInstance = get(inst);
      let constructor = internalInstance.type;
    ...
    function findRenderedDOMComponentWithClass(root, className) {
      ...
      let all = scryRenderedDOMComponentsWithClass(root, className);
    ...
    function findRenderedDOMComponentWithTag(root, tagName) {
      ...
      let all = scryRenderedDOMComponentsWithTag(root, tagName);
    ...
    function findRenderedComponentWithType(root, componentType) {
      ...
      let all = scryRenderedComponentsWithType(root, componentType);
    ...
    let Simulate = {};
    let directDispatchEventTypes = new Set([
      'mouseEnter',
      'mouseLeave',
      'pointerEnter',
      'pointerLeave',
    ...
    function makeSimulator(eventType) {
      return function (domNode, eventData) {
        ...
        let reactName = 'on' + eventType[0].toUpperCase() + eventType.slice(1);
        let fakeNativeEvent = new Event();
        fakeNativeEvent.target = domNode;
        fakeNativeEvent.type = eventType.toLowerCase();
        let targetInst = getInstanceFromNode(domNode);
