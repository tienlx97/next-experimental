...
if (process.env.NODE_ENV !== 'production') {
  (function () {
    ...
    let React = require('next/dist/compiled/react-experimental');
    ...
    let ReactVersion = '18.3.0-canary-aec521a96-20231114';
    ...
    function writeStringChunk(destination, stringChunk) {
      ...
      let _textEncoder$encodeIn = textEncoder.encodeInto(stringChunk, target);
      let read = _textEncoder$encodeIn.read;
      let written = _textEncoder$encodeIn.written;
    ...
    function writeToDestination(destination, view) {
      let currentHasCapacity = destination.write(view);
    ...
    function stringToPrecomputedChunk(content) {
      let precomputedChunk = textEncoder.encode(content);
    ...
    // -----------------------------------------------------------------------------
    let enableFloat = true; // Enables unstable_useMemoCache hook, intended as a compilation target for
    ...
    let ATTRIBUTE_NAME_START_CHAR =
      ':A-Z_a-z\\u00C0-\\u00D6\\u00D8-\\u00F6\\u00F8-\\u02FF\\u0370-\\u037D\\u037F-\\u1FFF\\u200C-\\u200D\\u2070-\\u218F\\u2C00-\\u2FEF\\u3001-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFFD';
    /* eslint-enable max-len */

    let ATTRIBUTE_NAME_CHAR =
      ATTRIBUTE_NAME_START_CHAR +
      '\\-.0-9\\u00B7\\u0300-\\u036F\\u203F-\\u2040';
    let VALID_ATTRIBUTE_NAME_REGEX = new RegExp(
    ...
    function validateProperty$1(tagName, name) {
      {
        ...
        if (rARIACamel$1.test(name)) {
          ...
          let correctName = ariaProperties.hasOwnProperty(ariaName)
            ? ariaName
        ...
        if (rARIA$1.test(name)) {
          ...
          let standardName = ariaProperties.hasOwnProperty(lowerCasedName)
            ? lowerCasedName
            : null; // If this is an aria-* attribute, but is not listed in the known DOM
          // DOM properties, then it is an invalid aria-* attribute.
    ...
    function validateProperties$2(type, props) {
      {
        ...
        for (let key in props) {
          let isValid = validateProperty$1(type, key);
        ...
        let unknownPropString = invalidProps
          .map(function (prop) {
            return '`' + prop + '`';
          })
          .join(', ');
    ...
    function validateProperty(tagName, name, value, eventRegistry) {
      {
        ...
        if (eventRegistry != null) {
          ...
          let registrationName = possibleRegistrationNames.hasOwnProperty(
            lowerCasedName
          )
    ...
    function warnUnknownProperties(type, props, eventRegistry) {
      {
        ...
        for (let key in props) {
          let isValid = validateProperty(type, key, props[key], eventRegistry);
        ...
        let unknownPropString = unknownProps
          .map(function (prop) {
            return '`' + prop + '`';
          })
          .join(', ');
    ...
    let NotPending = Object.freeze(sharedNotPendingObject);

    let ReactDOMSharedInternals =
      ReactDOM.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED;

    let ReactDOMCurrentDispatcher = ReactDOMSharedInternals.Dispatcher;
    let ReactDOMServerDispatcher = {
    ...
    let ScriptStreamingFormat = 0;
    let DataStreamingFormat = 1;
    let NothingSent =
      /*                      */
      0;
    let SentCompleteSegmentFunction =
      /*      */
      1;
    let SentCompleteBoundaryFunction =
      /*     */
      2;
    let SentClientRenderFunction =
      /*         */
      4;
    let SentStyleInsertionFunction =
      /*       */
      8;
    let SentFormReplayingRuntime =
      /*         */
    ...
    let EXISTS = null; // This constant is to mark preloads that have no unique credentials
    ...
    let dataElementQuotedEnd = stringToPrecomputedChunk('"></template>');
    let startInlineScript = stringToPrecomputedChunk('<script>');
    let endInlineScript = stringToPrecomputedChunk('</script>');
    let startScriptSrc = stringToPrecomputedChunk('<script src="');
    let startModuleSrc = stringToPrecomputedChunk(
      '<script type="module" src="'
    );
    let scriptNonce = stringToPrecomputedChunk('" nonce="');
    let scriptIntegirty = stringToPrecomputedChunk('" integrity="');
    let scriptCrossOrigin = stringToPrecomputedChunk('" crossorigin="');
    let endAsyncScript = stringToPrecomputedChunk('" async=""></script>');
    /**
    ...
    let importMapScriptStart = stringToPrecomputedChunk(
      '<script type="importmap">'
    );
    let importMapScriptEnd = stringToPrecomputedChunk('</script>'); // Since we store headers as strings we deal with their length in utf16 code units
    ...
    let DEFAULT_HEADERS_CAPACITY_IN_UTF16_CODE_UNITS = 2000; // Allows us to keep track of what we've already written so we can refer back to it.
    ...
    function createRenderState(
      ...
      let inlineScriptWithNonce =
        nonce === undefined
          ? startInlineScript
          : stringToPrecomputedChunk(
              '<script nonce="' + escapeTextForBrowser(nonce) + '">'
            );
      let idPrefix = resumableState.idPrefix;
      let bootstrapChunks = [];
      let externalRuntimeScript = null;
      let bootstrapScriptContent = resumableState.bootstrapScriptContent;
      let bootstrapScripts = resumableState.bootstrapScripts;
      let bootstrapModules = resumableState.bootstrapModules;
      ...
      if (bootstrapScripts !== undefined) {
        for (let i = 0; i < bootstrapScripts.length; i++) {
          let scriptConfig = bootstrapScripts[i];
          let src = void 0;
          let crossOrigin = void 0;
          let integrity = void 0;
          let props = {
            rel: 'preload',
            as: 'script',
            fetchPriority: 'low',
      ...
      if (bootstrapModules !== undefined) {
        for (let _i = 0; _i < bootstrapModules.length; _i++) {
          let _scriptConfig = bootstrapModules[_i];

          let _src = void 0;
          let _crossOrigin = void 0;
          let _integrity = void 0;
    ...
    function createResumableState(
      ...
      let idPrefix = identifierPrefix === undefined ? '' : identifierPrefix;
    ...
    let ROOT_HTML_MODE = 0; // Used for the root most element tag.
    // We have a less than HTML_HTML_MODE check elsewhere. If you add more cases here, make sure it
    // still makes sense

    let HTML_HTML_MODE = 1; // Used for the <html> if it is at the top level.

    let HTML_MODE = 2;
    let SVG_MODE = 3;
    let MATHML_MODE = 4;
    let HTML_TABLE_MODE = 5;
    let HTML_TABLE_BODY_MODE = 6;
    let HTML_TABLE_ROW_MODE = 7;
    let HTML_COLGROUP_MODE = 8; // We have a greater than HTML_TABLE_MODE check elsewhere. If you add more cases here, make sure it
    // still makes sense
    ...
    let textSeparator = stringToPrecomputedChunk('<!-- -->');
    ...
    let styleNameCache = new Map();

    function processStyleName(styleName) {
      let chunk = styleNameCache.get(styleName);

      if (chunk !== undefined) {
        return chunk;
      }

      let result = stringToPrecomputedChunk(
    ...
    let styleAttributeStart = stringToPrecomputedChunk(' style="');
    let styleAssign = stringToPrecomputedChunk(':');
    let styleSeparator = stringToPrecomputedChunk(';');
    ...
    let attributeSeparator = stringToPrecomputedChunk(' ');
    let attributeAssign = stringToPrecomputedChunk('="');
    var attributeEnd = stringToPrecomputedChunk('"');
    let attributeEmptyString = stringToPrecomputedChunk('=""');
    ...
    function makeFormFieldPrefix(resumableState) {
      let id = resumableState.nextFormID++;
      return resumableState.idPrefix + id;
    } // Since this will likely be repeated a lot in the HTML, we use a more concise message
    // than on the client and hopefully it's googleable.

    let actionJavaScriptURL = stringToPrecomputedChunk(
      escapeTextForBrowser(
        // eslint-disable-next-line no-script-url
        "javascript:throw new Error('A React form was unexpectedly submitted.')"
      )
    );
    let startHiddenInputChunk = stringToPrecomputedChunk(
      '<input type="hidden"'
    );
    ...
    function pushFormActionAttribute(
      ...
      if (typeof formAction === 'function') {
        ...
        let customAction = formAction.$$FORM_ACTION;

        if (typeof customAction === 'function') {
          // This action has a custom progressive enhancement form that can submit the form
          // back to the server if it's invoked before hydration. Such as a Server Action.
          let prefix = makeFormFieldPrefix(resumableState);
          let customFields = formAction.$$FORM_ACTION(prefix);
    ...
    function pushAttribute(target, name, value) {
      ...
      switch (name) {
        ...
        case 'formAction': {
          ...
          let sanitizedValue = sanitizeURL('' + value);
          target.push(
            attributeSeparator,
            stringToChunk(name),
            attributeAssign,
        ...
        case 'xlinkHref': {
          ...
          let _sanitizedValue = sanitizeURL('' + value);
        ...
        default:
          ...
          if (isAttributeNameSafe(attributeName)) {
            ...
            switch (typeof value) {
              ...
              case 'boolean': {
                let prefix = attributeName.toLowerCase().slice(0, 5);
    ...
    let endOfStartTag = stringToPrecomputedChunk('>');
    ...
    let selectedMarkerAttribute = stringToPrecomputedChunk(' selected=""');

    function pushStartOption(target, props, formatContext) {
      let selectedValue = formatContext.selectedValue;
    ...
    let formReplayingRuntimeScript = stringToPrecomputedChunk(formReplaying);
    ...
    let formStateMarkerIsMatching = stringToPrecomputedChunk('<!--F!-->');
    let formStateMarkerIsNotMatching = stringToPrecomputedChunk('<!--F-->');
    ...
    function pushInput(target, props, resumableState, renderState) {
      ...
      let formData = pushFormActionAttribute(
        target,
        resumableState,
        renderState,
        formAction,
    ...
    function pushLink(
    ...
    ) {
      {
        let rel = props.rel;
        let href = props.href;
        let precedence = props.precedence;
        ...
        if (props.rel === 'stylesheet') {
          ...
          if (
            ...
            let hasKey = resumableState.styleResources.hasOwnProperty(key);
            let resourceState = hasKey
            ...
            if (resourceState !== EXISTS) {
              ...
              if (resourceState) {
                ...
                let preloadState = resourceState;

                if (preloadState.length === 2) {
                  adoptPreloadCredentials(resource.props, preloadState);
                }

                let preloadResource = renderState.preloads.stylesheets.get(key);
              ...
              if (styleQueue) {
                let _resource = styleQueue.sheets.get(key);
    ...
    function pushStyle(
    ...
    ) {
      {
        if (hasOwnProperty.call(props, 'children')) {
          ...
          let child = Array.isArray(children)
            ? children.length < 2
              ? children[0]
              : null
            : children;
    ...
    function pushImg(
      ...
      if (
        ...
        // resumableState.
        let sizes = typeof props.sizes === 'string' ? props.sizes : undefined;
        let key = getImageResourceKey(src, srcSet, sizes);
        let promotablePreloads = renderState.preloads.images;
        ...
        if (resource) {
          ...
          let crossOrigin = getCrossOriginString(props.crossOrigin);
          let headers = renderState.headers;
          let header;
    ...
    function pushScript(
    ...
    ) {
      {
        ...
        let src = props.src;
        let key = getResourceKey(src); // We can make this <script> into a ScriptResource
        ...
        let hasKey = resources.hasOwnProperty(key);
        let resourceState = hasKey ? resources[key] : undefined;
        ...
        if (resourceState !== EXISTS) {
          ...
          if (resourceState) {
            ...
            let preloadResource = preloads.get(key);
          ...
          let resource = []; // Add to the script flushing queue
    ...
    function writeBootstrap(destination, renderState) {
      let bootstrapChunks = renderState.bootstrapChunks;
      let i = 0;
      ...
      if (i < bootstrapChunks.length) {
        let lastChunk = bootstrapChunks[i];
    ...
    let placeholder1 = stringToPrecomputedChunk('<template id="');
    let placeholder2 = stringToPrecomputedChunk('"></template>');
    function writePlaceholder(destination, renderState, id) {
      writeChunk(destination, placeholder1);
      writeChunk(destination, renderState.placeholderPrefix);
      let formattedID = stringToChunk(id.toString(16));
      writeChunk(destination, formattedID);
      return writeChunkAndReturn(destination, placeholder2);
    } // Suspense boundaries are encoded as comments.

    let startCompletedSuspenseBoundary = stringToPrecomputedChunk('<!--$-->');
    let startPendingSuspenseBoundary1 = stringToPrecomputedChunk(
      '<!--$?--><template id="'
    );
    let startPendingSuspenseBoundary2 =
      stringToPrecomputedChunk('"></template>');
    let startClientRenderedSuspenseBoundary =
      stringToPrecomputedChunk('<!--$!-->');
    let endSuspenseBoundary = stringToPrecomputedChunk('<!--/$-->');
    let clientRenderedSuspenseBoundaryError1 =
      stringToPrecomputedChunk('<template');
    let clientRenderedSuspenseBoundaryErrorAttrInterstitial =
      stringToPrecomputedChunk('"');
    let clientRenderedSuspenseBoundaryError1A =
      stringToPrecomputedChunk(' data-dgst="');
    let clientRenderedSuspenseBoundaryError1B =
      stringToPrecomputedChunk(' data-msg="');
    let clientRenderedSuspenseBoundaryError1C =
      stringToPrecomputedChunk(' data-stck="');
    let clientRenderedSuspenseBoundaryError2 =
      stringToPrecomputedChunk('></template>');
    ...
    let startSegmentHTML = stringToPrecomputedChunk('<div hidden id="');
    let startSegmentHTML2 = stringToPrecomputedChunk('">');
    let endSegmentHTML = stringToPrecomputedChunk('</div>');
    let startSegmentSVG = stringToPrecomputedChunk(
      '<svg aria-hidden="true" style="display:none" id="'
    );
    let startSegmentSVG2 = stringToPrecomputedChunk('">');
    let endSegmentSVG = stringToPrecomputedChunk('</svg>');
    let startSegmentMathML = stringToPrecomputedChunk(
      '<math aria-hidden="true" style="display:none" id="'
    );
    let startSegmentMathML2 = stringToPrecomputedChunk('">');
    let endSegmentMathML = stringToPrecomputedChunk('</math>');
    let startSegmentTable = stringToPrecomputedChunk('<table hidden id="');
    let startSegmentTable2 = stringToPrecomputedChunk('">');
    let endSegmentTable = stringToPrecomputedChunk('</table>');
    let startSegmentTableBody = stringToPrecomputedChunk(
      '<table hidden><tbody id="'
    );
    let startSegmentTableBody2 = stringToPrecomputedChunk('">');
    let endSegmentTableBody = stringToPrecomputedChunk('</tbody></table>');
    let startSegmentTableRow = stringToPrecomputedChunk(
      '<table hidden><tr id="'
    );
    let startSegmentTableRow2 = stringToPrecomputedChunk('">');
    let endSegmentTableRow = stringToPrecomputedChunk('</tr></table>');
    let startSegmentColGroup = stringToPrecomputedChunk(
      '<table hidden><colgroup id="'
    );
    let startSegmentColGroup2 = stringToPrecomputedChunk('">');
    let endSegmentColGroup = stringToPrecomputedChunk('</colgroup></table>');
    ...
    let completeSegmentScript1Full = stringToPrecomputedChunk(
      completeSegment + '$RS("'
    );
    let completeSegmentScript1Partial = stringToPrecomputedChunk('$RS("');
    let completeSegmentScript2 = stringToPrecomputedChunk('","');
    let completeSegmentScriptEnd = stringToPrecomputedChunk('")</script>');
    let completeSegmentData1 = stringToPrecomputedChunk(
      '<template data-rsi="" data-sid="'
    );
    let completeSegmentData2 = stringToPrecomputedChunk('" data-pid="');
    let completeSegmentDataEnd = dataElementQuotedEnd;
    function writeCompletedSegmentInstruction(
      ...
      writeChunk(destination, renderState.segmentPrefix);
      let formattedID = stringToChunk(contentSegmentID.toString(16));
      writeChunk(destination, formattedID);
    ...
    let completeBoundaryScript1Full = stringToPrecomputedChunk(
      completeBoundary + '$RC("'
    );
    let completeBoundaryScript1Partial = stringToPrecomputedChunk('$RC("');
    let completeBoundaryWithStylesScript1FullBoth = stringToPrecomputedChunk(
      completeBoundary + completeBoundaryWithStyles + '$RR("'
    );
    let completeBoundaryWithStylesScript1FullPartial = stringToPrecomputedChunk(
      completeBoundaryWithStyles + '$RR("'
    );
    let completeBoundaryWithStylesScript1Partial =
      stringToPrecomputedChunk('$RR("');
    let completeBoundaryScript2 = stringToPrecomputedChunk('","');
    let completeBoundaryScript3a = stringToPrecomputedChunk('",');
    let completeBoundaryScript3b = stringToPrecomputedChunk('"');
    let completeBoundaryScriptEnd = stringToPrecomputedChunk(')</script>');
    let completeBoundaryData1 = stringToPrecomputedChunk(
      '<template data-rci="" data-bid="'
    );
    let completeBoundaryWithStylesData1 = stringToPrecomputedChunk(
      '<template data-rri="" data-bid="'
    );
    let completeBoundaryData2 = stringToPrecomputedChunk('" data-sid="');
    let completeBoundaryData3a = stringToPrecomputedChunk('" data-sty="');
    let completeBoundaryDataEnd = dataElementQuotedEnd;
    function writeCompletedBoundaryInstruction(
      ...
      let idChunk = stringToChunk(id.toString(16));
      writeChunk(destination, renderState.boundaryPrefix);
    ...
    let clientRenderScript1Full = stringToPrecomputedChunk(
      clientRenderBoundary + ';$RX("'
    );
    let clientRenderScript1Partial = stringToPrecomputedChunk('$RX("');
    let clientRenderScript1A = stringToPrecomputedChunk('"');
    let clientRenderErrorScriptArgInterstitial = stringToPrecomputedChunk(',');
    let clientRenderScriptEnd = stringToPrecomputedChunk(')</script>');
    let clientRenderData1 = stringToPrecomputedChunk(
      '<template data-rxi="" data-bid="'
    );
    let clientRenderData2 = stringToPrecomputedChunk('" data-dgst="');
    let clientRenderData3 = stringToPrecomputedChunk('" data-msg="');
    let clientRenderData4 = stringToPrecomputedChunk('" data-stck="');
    let clientRenderDataEnd = dataElementQuotedEnd;
    ...
    let regexForJSStringsInInstructionScripts = /[<\u2028\u2029]/g;
    ...
    let regexForJSStringsInScripts = /[&><\u2028\u2029]/g;
    ...
    let lateStyleTagResourceOpen1 = stringToPrecomputedChunk(
      '<style media="not all" data-precedence="'
    );
    let lateStyleTagResourceOpen2 = stringToPrecomputedChunk('" data-href="');
    let lateStyleTagResourceOpen3 = stringToPrecomputedChunk('">');
    let lateStyleTagTemplateClose = stringToPrecomputedChunk('</style>'); // Tracks whether the boundary currently flushing is flushign style tags or has any
    ...
    function flushStyleTagsLateForBoundary(styleQueue) {
      let rules = styleQueue.rules;
      let hrefs = styleQueue.hrefs;
    ...
    let styleTagResourceOpen1 = stringToPrecomputedChunk(
      '<style data-precedence="'
    );
    let styleTagResourceOpen2 = stringToPrecomputedChunk('" data-href="');
    var spaceSeparator = stringToPrecomputedChunk(' ');
    let styleTagResourceOpen3 = stringToPrecomputedChunk('">');
    let styleTagResourceClose = stringToPrecomputedChunk('</style>');

    function flushStylesInPreamble(styleQueue, precedence) {
      let hasStylesheets = styleQueue.sheets.size > 0;
      styleQueue.sheets.forEach(flushStyleInPreamble, this);
      styleQueue.sheets.clear();
      let rules = styleQueue.rules;
      let hrefs = styleQueue.hrefs; // If we don't emit any stylesheets at this precedence we still need to maintain the precedence
    ...
    function preloadLateStyle(stylesheet) {
      if (stylesheet.state === PENDING$1) {
        ...
        let preloadProps = preloadAsStylePropsFromProps(
    ...
    function writePreamble(
      ...
      if (!willFlushAllSegments && renderState.externalRuntimeScript) {
        ...
        let _renderState$external = renderState.externalRuntimeScript;
        let src = _renderState$external.src;
        let chunks = _renderState$external.chunks;
        internalPreinitScript(resumableState, renderState, src, chunks);
      }

      let htmlChunks = renderState.htmlChunks;
      let headChunks = renderState.headChunks;
      ...
      let charsetChunks = renderState.charsetChunks;
      ...
      let preconnectChunks = renderState.preconnectChunks;
      ...
      let importMapChunks = renderState.importMapChunks;
      ...
      let preloadChunks = renderState.preloadChunks;
      ...
      let hoistableChunks = renderState.hoistableChunks;
    ...
    let arrayFirstOpenBracket = stringToPrecomputedChunk('[');
    let arraySubsequentOpenBracket = stringToPrecomputedChunk(',[');
    let arrayInterstitial = stringToPrecomputedChunk(',');
    let arrayCloseBracket = stringToPrecomputedChunk(']'); // This function writes a 2D array of strings to be embedded in javascript.
    // E.g.
    ...
    function writeStyleResourceDependencyInJS(
      ...
      // eslint-disable-next-line react-internal/safe-string-coercion
      let coercedHref = sanitizeURL('' + href);
      writeChunk(
        destination,
        stringToChunk(escapeJSObjectForInstructionScripts(coercedHref))
      );
      ...
      let coercedPrecedence = '' + precedence;
    ...
    function prefetchDNS(href) {
      ...
      let resumableState = getResumableState(request);
      let renderState = getRenderState(request);
      ...
      if (as && href) {
        switch (as) {
          case 'image': {
            ...
            let key = getImageResourceKey(href, imageSrcSet, imageSizes);
          ...
          default: {
            let _key3 = getResourceKey(href);

            let hasAsType = resumableState.unknownResources.hasOwnProperty(as);
            let resources;
            ...
            let _headers = renderState.headers;
    ...
    function preinitScript(src, options) {
      ...
      if (src) {
        let key = getResourceKey(src);
        let hasKey = resumableState.scriptResources.hasOwnProperty(key);
        let resourceState = hasKey
        ...
        if (resourceState !== EXISTS) {
          ...
          if (resourceState) {
            ...
            let preloadResource = renderState.preloads.scripts.get(key);
    ...
    function preinitModuleScript(src, options) {
      ...
      if (src) {
        let key = getResourceKey(src);
        let hasKey = resumableState.moduleScriptResources.hasOwnProperty(key);
        let resourceState = hasKey
          ? resumableState.moduleScriptResources[key]
          : undefined;
        ...
        if (resourceState !== EXISTS) {
          ...
          if (resourceState) {
            ...
            let preloadResource = renderState.preloads.moduleScripts.get(key);
    ...
    function getPrefetchDNSAsHeader(href) {
      let escapedHref = escapeHrefForLinkHeaderURLContext(href);
    ...
    function getPreloadAsHeader(href, as, params) {
      let escapedHref = escapeHrefForLinkHeaderURLContext(href);
      let escapedAs = escapeStringForLinkHeaderQuotedParamValueContext(
        as,
        'as'
      );
    ...
    let regexForHrefInLinkHeaderURLContext = /[<>\r\n]/g;
    ...
    let regexForLinkHeaderQuotedParamValueContext = /["';,\r\n]/g;
    ...
    function hoistResources(renderState, source) {
      let currentBoundaryResources = renderState.boundaryResources;
    ...
    function emitEarlyPreloads(renderState, resumableState, shellComplete) {
      let onHeaders = renderState.onHeaders;
      ...
      if (onHeaders) {
        ...
        if (headers) {
          ...
          if (!shellComplete) {
            ...
            let queueIter = renderState.styles.values();
            ...
            outer: for (
              ...
              let sheets = queueStep.value.sheets;
              let sheetIter = sheets.values();
              ...
              for (
                ...
                let sheet = sheetStep.value;
                let props = sheet.props;
                let key = getResourceKey(props.href);
                let header = getStylesheetPreloadAsHeader(sheet); // We mutate the capacity b/c we don't want to keep checking if later headers will fit.
    ...
    let NotPendingTransition = NotPending;

    let requestStorage = new async_hooks.AsyncLocalStorage();
    ...
    let REACT_SERVER_CONTEXT_TYPE = Symbol.for('react.server_context');
    ...
    let REACT_DEBUG_TRACING_MODE_TYPE = Symbol.for('react.debug_trace_mode');
    let REACT_OFFSCREEN_TYPE = Symbol.for('react.offscreen');
    let REACT_LEGACY_HIDDEN_TYPE = Symbol.for('react.legacy_hidden');
    let REACT_CACHE_TYPE = Symbol.for('react.cache');
    let REACT_SERVER_CONTEXT_DEFAULT_VALUE_NOT_LOADED = Symbol.for(
      'react.default_value'
    );
    ...
    function describeNativeComponentFrame(fn, construct) {
      ...
      reentry = true;
      let previousPrepareStackTrace = Error.prepareStackTrace; // $FlowFixMe[incompatible-type] It does accept undefined.
        ...
        DetermineComponentFrameRoot: function () {
          ...
          try {
            ...
            if (construct) {
              ...
              let maybePromise = fn(); // If the function component returns a promise, it's likely an async
    ...
    function getMaskedContext(type, unmaskedContext) {
      {
        ...
        {
          let name = getComponentNameFromType(type) || 'Unknown';
    ...
    function processChildContext(
    ...
    ) {
      {
        ...
        if (typeof instance.getChildContext !== 'function') {
          {
            let componentName = getComponentNameFromType(type) || 'Unknown';
    ...
    let rootContextSnapshot = null; // We assume that this runtime owns the "current" field on all ReactContext instances.
    ...
    function popToNearestCommonAncestor(prev, next) {
      if (prev === next);
        ...
        popNode(prev);
        let parentPrev = prev.parent;
        let parentNext = next.parent;
    ...
    function switchContext(newSnapshot) {
      ...
      let prev = currentActiveSnapshot;
      let next = newSnapshot;
    ...
    function pushProvider(context, nextValue) {
      ...
      let prevNode = currentActiveSnapshot;
      let newNode = {
        parent: prevNode,
    ...
    function popProvider(context) {
      let prevSnapshot = currentActiveSnapshot;
      ...
      {
        let value = prevSnapshot.parentValue;
    ...
    let didWarnAboutNoopUpdateForComponent = {};
    let didWarnAboutDeprecatedWillMount = {};
    ...
    function warnOnUndefinedDerivedState(type, partialState) {
      {
        if (partialState === undefined) {
          let componentName = getComponentNameFromType(type) || 'Component';
    ...
    function constructClassInstance(ctor, props, maskedLegacyContext) {
...

      {
        if (
          ...
          let componentName = getComponentNameFromType(ctor) || 'Component';
        ...
        if (
          ...
          if (
            ...
            let _componentName = getComponentNameFromType(ctor) || 'Component';
    ...
    function checkClassInstance(instance, ctor, newProps) {
      {
        let name = getComponentNameFromType(ctor) || 'Component';
    ...
    function processUpdateQueue(
      ...
      if (
        ...
        let oldQueue = internalInstance.queue;
        let oldReplace = internalInstance.replace;
        internalInstance.queue = null;
    ...
    function mountClassInstance(instance, ctor, newProps, maskedLegacyContext) {
      ...
      let initialState = instance.state !== undefined ? instance.state : null;
    ...
    let emptyTreeContext = {
      id: 1,
      overflow: '',
    };
    function getTreeId(context) {
      let overflow = context.overflow;
      let idWithLeadingBit = context.id;
      let id = idWithLeadingBit & ~getLeadingBit(idWithLeadingBit);
      return id.toString(32) + overflow;
    }
    function pushTreeContext(baseContext, totalChildren, index) {
      let baseIdWithLeadingBit = baseContext.id;
      let baseOverflow = baseContext.overflow; // The leftmost 1 marks the end of the sequence, non-inclusive. It's not part
      // of the id; we use it to account for leading 0s.

      let baseLength = getBitLength(baseIdWithLeadingBit) - 1;
      let baseId = baseIdWithLeadingBit & ~(1 << baseLength);
      let slot = index + 1;
      let length = getBitLength(totalChildren) + baseLength; // 30 is the max length we can store without overflowing, taking into
      ...
      if (length > 30) {
        ...
        let numberOfOverflowBits = baseLength - (baseLength % 5); // Then create a bitmask that selects only those bits.

        let newOverflowBits = (1 << numberOfOverflowBits) - 1; // Select the bits, and convert them to a base 32 string.

        let newOverflow = (baseId & newOverflowBits).toString(32); // Now we can remove those bits from the base id.

        let restOfBaseId = baseId >> numberOfOverflowBits;
        let restOfBaseLength = baseLength - numberOfOverflowBits; // Finally, encode the rest of the bits using the normal algorithm. Because
        // we made more room, this time it won't overflow.

        let restOfLength = getBitLength(totalChildren) + restOfBaseLength;
        let restOfNewBits = slot << restOfBaseLength;
        let id = restOfNewBits | restOfBaseId;
        let overflow = newOverflow + baseOverflow;
        return {
        ...
        // Normal path
        let newBits = slot << baseLength;

        let _id = newBits | baseId;

        let _overflow = baseOverflow;
        return {
          id: (1 << length) | _id,
          overflow: _overflow,
        };
    ...
    let SuspenseException = new Error(
    ...
    function trackUsedThenable(thenableState, thenable, index) {
      let previous = thenableState[index];
      ...
      switch (thenable.status) {
        case 'fulfilled': {
          let fulfilledValue = thenable.value;
          return fulfilledValue;
        }

        case 'rejected': {
          let rejectedError = thenable.reason;
          throw rejectedError;
        }
    ...
    let objectIs = typeof Object.is === 'function' ? Object.is : is; // $FlowFixMe[method-unbinding]
    ...
    function checkDidRenderIdHook() {
      ...
      let didRenderIdHook = localIdCounter !== 0;
    ...
    function useReducer(reducer, initialArg, init) {
      ...
      if (isReRender) {
        ...
        let _queue = (workInProgressHook.queue = {
          last: null,
          dispatch: null,
        });

        let _dispatch = (_queue.dispatch = dispatchAction.bind(
          null,
    ...
    function useMemo(nextCreate, deps) {
      ...
      if (workInProgressHook !== null) {
        let prevState = workInProgressHook.memoizedState;
    ...
    function useRef(initialValue) {
      currentlyRenderingComponent = resolveCurrentlyRenderingComponent();
      workInProgressHook = createWorkInProgressHook();
      let previousRef = workInProgressHook.memoizedState;
    ...
    function createPostbackFormStateKey(
      ...
      if (permalink !== undefined) {
        ...
        let keyPath = [componentKeyPath, null, hookIndex]; // Key paths are hashed to reduce the size. It does not need to be secure,
        // and it's more important that it's fast than that it's completely
        // collision-free.

        let keyPathHash = createFastHash(JSON.stringify(keyPath));
    ...
    function useFormState(action, initialState, permalink) {
      ...
      let formStateHookIndex = formStateCounter++;
      let request = currentlyRenderingRequest; // $FlowIgnore[prop-missing]

      let formAction = action.$$FORM_ACTION;
      ...
      if (typeof formAction === 'function') {
        ...
        let componentKeyPath = currentlyRenderingKeyPath;
        let postbackFormState = getFormState(request); // $FlowIgnore[prop-missing]

        let isSignatureEqual = action.$$IS_SIGNATURE_EQUAL;

        if (
          postbackFormState !== null &&
          typeof isSignatureEqual === 'function'
        ) {
          let postbackKey = postbackFormState[1];
          let postbackReferenceId = postbackFormState[2];
          let postbackBoundArity = postbackFormState[3];
        ...
        let boundAction = action.bind(null, state); // Wrap the action so the return value is void.
        ...
        if (typeof boundAction.$$FORM_ACTION === 'function') {
          ...
          dispatch.$$FORM_ACTION = function (prefix) {
            let metadata = boundAction.$$FORM_ACTION(prefix); // Override the action URL
        ...
        // Bind the state to the first argument of the action.
        let _boundAction = action.bind(null, initialState); // Wrap the action so the return value is void.

        let _dispatch2 = function (payload) {
    ...
    function useId() {
      let task = currentlyRenderingTask;
      let treeId = getTreeId(task.treeContext);
      let resumableState = currentResumableState;
    ...
    let ReactCurrentCache = ReactSharedInternals.ReactCurrentCache;
    let ReactDebugCurrentFrame = ReactSharedInternals.ReactDebugCurrentFrame; // Linked list representing the identity of a component given the component/tag name and key.
    // The name might be minified but we assume that it's going to be the same generated name. Typically
    // because it's just the same compiled output in practice.
    // resume with segmentID at the index

    let CLIENT_RENDERED = 4; // if it errors or infinitely suspends
    ...
    let OPEN = 0;
    let CLOSING = 1;
    let CLOSED = 2; // This is a default heuristic for how to split up the HTML content into progressive
    ...
    function createRequest(
      ...
      let rootSegment = createPendingSegment(
        request,
        0,
        null,
    ...
    function resumeRequest(
      ...
      if (typeof postponedState.replaySlots === 'number') {
        let resumedId = postponedState.replaySlots; // We have a resume slot at the very root. This is effectively just a full rerender.
        ...
        let _rootTask = createRenderTask(
          request,
          null,
          children,
          -1,
    ...
    function resolveRequest() {
      ...
      {
        let store = requestStorage.getStore();
    ...
    function pingTask(request, task) {
      let pingedTasks = request.pingedTasks;
    ...
    function logRecoverableError(request, error) {
      // If this callback errors, we intentionally let that error bubble up to become a fatal error
      // so that someone fixes the error reporting instead of hiding it.
      let errorDigest = request.onError(error);
    ...
    function fatalError(request, error) {
      ...
      let onShellError = request.onShellError;
      onShellError(error);
      let onFatalError = request.onFatalError;
      onFatalError(error);
    ...
    function renderSuspenseBoundary(request, someTask, keyPath, props) {
      if (someTask.replay !== null) {
        ...
        let _prevKeyPath = someTask.keyPath;
        someTask.keyPath = keyPath;
        let _content = props.children;
      ...
      let prevKeyPath = task.keyPath;
      let parentBoundary = task.blockedBoundary;
      let parentSegment = task.blockedSegment; // Each time we enter a suspense boundary, we split out into a new segment for
      ...
      let fallbackAbortSet = new Set();
      let newBoundary = createSuspenseBoundary(request, fallbackAbortSet);

      if (request.trackedPostpones !== null) {
        newBoundary.trackedContentKeyPath = keyPath;
      }

      let insertionIndex = parentSegment.chunks.length; // The children of the boundary segment is actually the fallback.

      let boundarySegment = createPendingSegment(
        request,
        insertionIndex,
        newBoundary,
      ...
      let contentRootSegment = createPendingSegment(
        request,
        0,
        null,
        task.formatContext, // boundaries never require text embedding at their edges because comment nodes bound them
      ...
      let fallbackKeyPath = [keyPath[0], 'Suspense Fallback', keyPath[2]];
      let trackedPostpones = request.trackedPostpones;

      if (trackedPostpones !== null) {
        // We create a detached replay node to track any postpones inside the fallback.
        let fallbackReplayNode = [
          fallbackKeyPath[1],
          fallbackKeyPath[2],
          [],
          null,
        ...
        if (newBoundary.status === POSTPONED) {
          // This must exist now.
          let boundaryReplayNode = trackedPostpones.workingMap.get(keyPath);
      ...
      let suspendedFallbackTask = createRenderTask(
        request,
        null,
        fallback,
        -1,
    ...
    function replaySuspenseBoundary(
      ...
      pushBuiltInComponentStackInDEV(task, 'Suspense');
      let prevKeyPath = task.keyPath;
      let previousReplaySet = task.replay;
      let parentBoundary = task.blockedBoundary;
      let content = props.children;
      let fallback = props.fallback;
      let fallbackAbortSet = new Set();
      let resumedBoundary = createSuspenseBoundary(request, fallbackAbortSet);
      ...
      let fallbackKeyPath = [keyPath[0], 'Suspense Fallback', keyPath[2]]; // We create suspended task for the fallback because we don't want to actually work
      // on it yet in case we finish the main content, so we queue for later.
      ...
      let suspendedFallbackTask = createReplayTask(
        request,
        null,
        fallbackReplay,
        fallback,
    ...
    function renderHostElement(request, task, keyPath, type, props) {
      ...
      if (segment === null) {
        // Replay
        let children = props.children; // TODO: Make this a Config for replaying.

        let prevContext = task.formatContext;
        let prevKeyPath = task.keyPath;
        ...
        // Render
        let _children = pushStartInstance(
          segment.chunks,
          type,
          props,
        ...
        let _prevContext = task.formatContext;
        let _prevKeyPath2 = task.keyPath;
    ...
    function finishClassComponent(
...

      {
        ...
        if (childContextTypes !== null && childContextTypes !== undefined) {
          let previousContext = task.legacyContext;
          let mergedContext = processChildContext(
            instance,
            Component,
            previousContext,
            childContextTypes
    ...
    function renderClassComponent(request, task, keyPath, Component, props) {
      ...
      let maskedContext = getMaskedContext(Component, task.legacyContext);
      let instance = constructClassInstance(Component, props, maskedContext);
    ...
    function renderIndeterminateComponent(
...

      {
        if (
          ...
          let componentName = getComponentNameFromType(Component) || 'Unknown';
      ...
      let formStateCount = getFormStateCount();
      let formStateMatchingIndex = getFormStateMatchingIndex();

      {
        ...
        if (
          ...
          let _componentName = getComponentNameFromType(Component) || 'Unknown';
    ...
    function finishFunctionComponent(
      ...
      if (hasId) {
        ...
        // a single "child" slot.
        let prevTreeContext = task.treeContext;
        let totalChildren = 1;
        let index = 0; // Modify the id context. Because we'll need to reset this if something
    ...
    function renderMemo(
      ...
      let resolvedProps = resolveDefaultProps(innerType, props);
      renderElement(
        request,
        task,
        keyPath,
    ...
    function renderContextConsumer(request, task, keyPath, context, props) {
      ...
      let newValue = readContext$1(context);
    ...
    function renderElement(
      ...
      switch (type) {
        ...
        case REACT_SUSPENSE_LIST_TYPE: {
          ...
          let _prevKeyPath3 = task.keyPath;
          task.keyPath = keyPath;
          renderNodeDestructive(request, task, null, props.children, -1);
        ...
        case REACT_SCOPE_TYPE: {
          {
            let _prevKeyPath4 = task.keyPath;
            task.keyPath = keyPath;
            renderNodeDestructive(request, task, null, props.children, -1);
    ...
    function resumeNode(request, task, segmentId, node, childIndex) {
      let prevReplay = task.replay;
      let blockedBoundary = task.blockedBoundary;
      let resumedSegment = createPendingSegment(
        request,
        0,
        null,
        task.formatContext,
    ...
    function replayElement(
      ...
      for (let i = 0; i < replayNodes.length; i++) {
        ...
        if (node.length === 4) {
          ...
          let childNodes = node[2];
          let childSlots = node[3];
          let currentNode = task.node;
          task.replay = {
            nodes: childNodes,
            slots: childSlots,
            pendingTasks: 1,
    ...
    function renderNodeDestructiveImpl(
      ...
      if (task.replay !== null && typeof task.replay.slots === 'number') {
        ...
        let resumeSegmentID = task.replay.slots;
      ...
      if (typeof node === 'object' && node !== null) {
        switch (node.$$typeof) {
          case REACT_ELEMENT_TYPE: {
            ...
            let keyPath = [task.keyPath, name, keyOrIndex];
          ...
          case REACT_LAZY_TYPE: {
            let lazyNode = node;
            let payload = lazyNode._payload;
            let init = lazyNode._init;
            let resolvedNode;
        ...
        let maybeUsable = node;

        if (typeof maybeUsable.then === 'function') {
          let thenable = maybeUsable;
          return renderNodeDestructiveImpl(
            request,
            task,
            null,
        ...
        let childString = Object.prototype.toString.call(node);
        throw new Error(
      ...
      if (typeof node === 'number') {
        let _segment = task.blockedSegment;
    ...
    function replayFragment(request, task, children, childIndex) {
      ...
      // this fragment.
      let replay = task.replay;
      let replayNodes = replay.nodes;

      for (let j = 0; j < replayNodes.length; j++) {
        let node = replayNodes[j];
    ...
    function renderChildrenArray(request, task, children, childIndex) {
      ...
      if (task.replay !== null) {
        ...
        let resumeSlots = task.replay.slots;
        ...
        if (resumeSlots !== null && typeof resumeSlots === 'object') {
          for (let i = 0; i < totalChildren; i++) {
            ...
            let resumeSegmentID = resumeSlots[i]; // TODO: If this errors we should still continue with the next sibling.
      ...
      for (let _i = 0; _i < totalChildren; _i++) {
        let _node = children[_i];
    ...
    function trackPostpone(request, trackedPostpones, task, segment) {
      ...
      let keyPath = task.keyPath;
      let boundary = task.blockedBoundary;
      ...
      if (boundary !== null && boundary.status === PENDING) {
        ...
        let boundaryKeyPath = boundary.trackedContentKeyPath;
        ...
        let fallbackReplayNode = boundary.trackedFallbackNode;
        let children = [];
        ...
        if (boundaryKeyPath === keyPath && task.childIndex === -1) {
          ...
          let _boundaryNode = trackedPostpones.workingMap.get(boundaryKeyPath);
          ...
          if (_boundaryNode === undefined) {
            ...
            let suspenseBoundary = _boundaryNode;
      ...
      if (task.childIndex === -1) {
        ...
        if (keyPath === null) {
          ...
          let workingMap = trackedPostpones.workingMap;
        ...
        if (keyPath === null) {
          ...
          let _workingMap = trackedPostpones.workingMap;

          let _resumableNode = _workingMap.get(keyPath);
    ...
    function injectPostponedHole(request, task, reason) {
      ...
      let segment = task.blockedSegment;
      let insertionIndex = segment.chunks.length;
      let newSegment = createPendingSegment(
        request,
        insertionIndex,
        null,
    ...
    function renderNode(request, task, node, childIndex) {
      ...
      // process.
      let previousFormatContext = task.formatContext;
      let previousLegacyContext = task.legacyContext;
      let previousContext = task.context;
      let previousKeyPath = task.keyPath;
      let previousTreeContext = task.treeContext;
      ...
      if (segment === null) {
        ...
        try {
          ...
          if (typeof x === 'object' && x !== null) {
            ...
            if (typeof x.then === 'function') {
              let wakeable = x;
              let thenableState = getThenableStateAfterSuspending();
        ...
        // Render
        let childrenLength = segment.children.length;
        let chunkLength = segment.chunks.length;
        ...
        try {
          ...
          if (typeof x === 'object' && x !== null) {
            ...
            if (typeof x.then === 'function') {
              ...
              let _thenableState = getThenableStateAfterSuspending();
            ...
            if (
              ...
              let postponeInstance = x;
              let trackedPostpones = request.trackedPostpones;
              let postponedSegment = injectPostponedHole(
                request,
    ...
    function abortRemainingSuspenseBoundary(
      ...
      let resumedBoundary = createSuspenseBoundary(request, new Set());
      resumedBoundary.parentFlushed = true; // We restore the same id of this boundary as was used during prerender.
      ...
      {
        let errorPrefix = 'The server did not finish this Suspense boundary: ';
        let errorMessage;
        ...
        let previousTaskInDev = currentTaskInDEV;
        currentTaskInDEV = null;
    ...
    function abortRemainingReplayNodes(
      ...
      for (let i = 0; i < nodes.length; i++) {
        ...
        if (node.length === 4) {
          ...
          let rootSegmentID = boundaryNode[5];
        ...
        if (request.status !== CLOSING && request.status !== CLOSED) {
          ...
          if (replay === null) {
            ...
            if (replay.pendingTasks === 0 && replay.nodes.length > 0) {
              let errorDigest = logRecoverableError(request, error);
              abortRemainingReplayNodes(
                request,
                null,
                replay.nodes,
    ...
    function completeShell(request) {
      ...
      let onShellReady = request.onShellReady;
      onShellReady();
    } // I extracted this function out because we want to ensure we consistently emit preloads before
    // transitioning to the next request stage and this transition can happen in multiple places in this
    // implementation.
    ...
    function completeAll(request) {
      ...
      let onAllReady = request.onAllReady;
      onAllReady();
    }
    ...
    function queueCompletedSegment(boundary, segment) {
      if (
        ...
        let childSegment = segment.children[0];
        ...
        let completedSegments = boundary.completedSegments;
    ...
    function retryRenderTask(request, task, segment) {
      ...
      try {
        ...
        let prevThenableState = task.thenableState;
    ...
    function performWork(request) {
      ...
      let prevContext = getActiveContext();
      ...
      let prevRequest = currentRequest;
      ...
      let prevResumableState = currentResumableState;
    ...
    function flushSubtree(request, destination, segment) {
      ...
      switch (segment.status) {
        ...
        case POSTPONED: {
          let segmentID = segment.id; // When this segment finally completes it won't be embedded in text since it will flush separately
        ...
        case COMPLETED: {
          ...
          for (let childIdx = 0; childIdx < children.length; childIdx++) {
            let nextChild = children[childIdx]; // Write all the chunks up until the next child.
    ...
    function flushSegment(request, destination, segment) {
      ...
      if (boundary.status === CLIENT_RENDERED) {
        ...
        let contentSegment = completedSegments[0];
    ...
    function flushCompletedBoundary(request, destination, boundary) {
      ...
      for (; i < completedSegments.length; i++) {
        let segment = completedSegments[i];
    ...
    function flushPartiallyCompletedSegment(
      ...
      if (segmentID === -1) {
        ...
        let rootSegmentID = (segment.id = boundary.rootSegmentID);
    ...
    function flushCompletedQueues(request, destination) {
      ...
      try {
        ...
        let i;
        let completedRootSegment = request.completedRootSegment;
        ...
        let clientRenderedBoundaries = request.clientRenderedBoundaries;

        for (i = 0; i < clientRenderedBoundaries.length; i++) {
          let boundary = clientRenderedBoundaries[i];
        ...
        let completedBoundaries = request.completedBoundaries;

        for (i = 0; i < completedBoundaries.length; i++) {
          let _boundary = completedBoundaries[i];
        ...
        let partialBoundaries = request.partialBoundaries;

        for (i = 0; i < partialBoundaries.length; i++) {
          let _boundary2 = partialBoundaries[i];
        ...
        let largeBoundaries = request.completedBoundaries;

        for (i = 0; i < largeBoundaries.length; i++) {
          let _boundary3 = largeBoundaries[i];
    ...
    function enqueueEarlyPreloadsAfterInitialWork(request) {
      let shellComplete = request.pendingRootTasks === 0;
      emitEarlyPreloads(
        request.renderState,
        request.resumableState,
        shellComplete
    ...
    function abort(request, reason) {
      try {
        let abortableTasks = request.abortableTasks;
    ...
    function createRequestImpl(children, options) {
      let resumableState = createResumableState(
    ...
    function renderToPipeableStream(children, options) {
      let request = createRequestImpl(children, options);
    ...
    function resumeToPipeableStream(children, postponedState, options) {
      let request = resumeRequestImpl(children, postponedState, options);
      let hasStartedFlowing = false;
      startWork(request);
      return {
        pipe: function (destination) {
